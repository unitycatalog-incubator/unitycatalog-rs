...
pub(crate) fn generate_client_ts(services: &[ServiceHandler<'_>]) -> String {
    ...
    // Generate the main UnityCatalogClient class
...
fn generate_imports(services: &[ServiceHandler<'_>]) -> String {
    ...
    let mut native_classes: Vec<String> =
        vec!["NapiUnityCatalogClient as NativeClient".to_string()];
    for service in services {
...
fn generate_resource_client_class(service: &ServiceHandler<'_>) -> Option<String> {
    ...
    // Generate child resource accessor methods
    // We look for services whose resource pattern suggests they are children
    // of this resource. This is handled below in the main client.
...
fn generate_unity_catalog_client(services: &[ServiceHandler<'_>]) -> String {
    ...
    format!(
        r#"export class UnityCatalogClient {{
